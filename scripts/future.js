...
function displayCategories() {
    ...
    allCategories = eventosFuturos.map(e => e.category)
...
function filtro() {
    ...
    if (checkSelected.length > 0 && textFilter !== '') {
        checkSelected.forEach(category => {
            dataDisplay.push(...eventosFuturos.filter(eventos =>
        ...
        checkSelected.forEach(category => {
            dataDisplay.push(...eventosFuturos.filter(eventos => eventos.category.toLowerCase() == category))
        })
    }
    else if (checkSelected.length == 0 && textFilter !== '') {
        dataDisplay.push(...eventosFuturos.filter(eventos => eventos.name.toLowerCase().includes(textFilter.trim().toLowerCase())))
    }
    else { dataDisplay.push(...eventosFuturos) }
