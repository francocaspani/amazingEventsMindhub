...
//Imprimiendo categorias en html
function displayCategories() {
    allCategories = dataFromAPI.events.map(e => e.category)
...
function filtro() {
    ...
    if (checkSelected.length > 0 && textFilter !== '') {
        checkSelected.forEach(category => {
            dataDisplay.push(...dataFromAPI.events.filter(eventos =>
        ...
        checkSelected.forEach(category => {
            dataDisplay.push(...dataFromAPI.events.filter(eventos => eventos.category.toLowerCase() == category))
        })
    }
    else if (checkSelected.length == 0 && textFilter !== '') {
        dataDisplay.push(...dataFromAPI.events.filter(eventos => eventos.name.toLowerCase().includes(textFilter.trim().toLowerCase())))
    }
    else { dataDisplay.push(...dataFromAPI.events) }
            ...
            <img src="${e.image}" class="card-img-top">
                        <div class="card-body">
                            ...
                            <div class="price">
                                ...
                                <a href="./pages/details.html?id=${e._id}" class="btn btn-secondary btn-details">Details</a>
                            </div>
                        </div>
            </div>
            `
